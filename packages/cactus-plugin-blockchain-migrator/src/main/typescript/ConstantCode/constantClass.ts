/* class ConstantClass {
  ...
  static async send(ctx: Context, value: number, addressFrom: string, addressTo: string, amount: number): Promise<boolean> { // tested
    if (value > amount) {
      let args = ['send', addressFrom, addressTo, amount.toString()];
      await ctx.stub.invokeChaincode('balance', args);
      value = value - amount;
      return true;
    }
    return false;
  ...
  static async transfer(ctx: Context, value: number, addressFrom: string, addressTo: string, amount: number): Promise<void> { // tested
    if (value > amount) {
      let args = ['send', addressFrom, addressTo, amount.toString()];
      await ctx.stub.invokeChaincode('balance', args);
      value = value - amount;
